...
export class PerformancesComponent implements OnInit {
  ...
  constructor(
    ...
    if (!isNullOrUndefined(router.getCurrentNavigation().extras.state)) {
      ...
      this.savedOptions.points =
        router.getCurrentNavigation().extras.state.points;
    }
  }
  ...
  ngOnInit() {
    if (!isNullOrUndefined(this.savedOptions)) {
      if (this.savedOptions.type !== "") {
        this.performanceForm.controls.typeSelect.setValue(
          this.savedOptions.type
        );
      } else {
        this.performanceForm.controls.typeSelect.setValue("i");
      }
      if (!isNullOrUndefined(this.savedOptions.points)) {
        this.performanceForm.controls.pointsInput.setValue(
          this.savedOptions.points.toString()
        );
      }
    } else {
      this.performanceForm.controls.typeSelect.setValue("i");
      ...
      .add(() => {
        this.dataSource = new MatTableDataSource(
          this.eventTableRows
            .map((event) => {
              if (
                isNullOrUndefined(this.performancesDic.m[event]) &&
                isNullOrUndefined(this.performancesDic.w[event])
              ) {
                return null;
              } else {
                return {
  ...
  private saveOptions() {
    localStorage.setItem(
      "performancesOptions",
      JSON.stringify(this.savedOptions)
    );
  }
}
