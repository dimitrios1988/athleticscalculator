  ...
  implements OnChanges, OnInit, AfterContentChecked, OnDestroy
{
  ...
  updateFormValidation() {
    ...
    if (
      this.selectedEvent.SupportsWind &&
    ...
    if (this.eventForm.controls.calculatePlacePointsCheckbox.value) {
      ...
      if (
        this.eventForm.controls.competitionTypeSelect.value.HasProgressToFinal
      ) {
  ...
  onCalculate(event?) {
    ...
    if (this.eventForm.controls.calculatePlacePointsCheckbox.value) {
      ...
      getPointsCmd.meetingCategory =
        this.eventForm.controls.meetingCategorySelect.value;
      getPointsCmd.place = Number(this.eventForm.controls.placeInput.value);
      getPointsCmd.progressToFinal = this.eventForm.controls
        .competitionTypeSelect.value.HasProgressToFinal
      ...
      .add(() => {
        if (this.eventForm.controls.datePicker.value) {
          ...
          if (this.eventForm.controls.datePoints.value == "All points") {
            ...
            this.totalPoints =
              this.totalPointsBeforeDeduction +
              Number(this.eventForm.controls.datePoints.value);
          }
        } else {
  ...
  private calculateDeductionPoints() {
    const diff = this.monthDiff(
      this.eventForm.controls.datePicker.value.toDate(),
      new Date()
    );
    if (this.selectedEvent.PointsDeductionStrategy.Max < diff) {
      ...
      if (
        !isNullOrUndefined(
          this.selectedEvent.PointsDeductionStrategy.MonthPoints
        )
      ) {
        if (
          !isNullOrUndefined(
            this.selectedEvent.PointsDeductionStrategy.MonthPoints[diff]
          )
        ) {
          this.eventForm.controls.datePoints.setValue(
            this.selectedEvent.PointsDeductionStrategy.MonthPoints[diff]
        ...
        try {
          this.bookmarksService.saveBookmark(
            ...
            this.eventForm,
            {
              ...
              totalPointsBeforeDeduction: this.totalPointsBeforeDeduction,
            }
          );
  ...
  public changePlaceValidation() {
    this.eventForm.controls.placeInput.clearValidators();
    if (
      !isNullOrUndefined(this.eventForm.controls.progressedToFinalCombo.value)
    ) {
